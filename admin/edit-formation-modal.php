...
<?php
    ...
    ?>
    <title>Modifier une modale</title>
</head>
...
<?php
    ...
    <div class="container bg-white shadow py-2">
        <h1>Modifier la modale</h1>
        <hr>
        <?php 
        //1- Connexion
        require_once('./core/connexion.php');
        // 2- Ecriture de la requête
        $sql = 'SELECT * FROM formation_modal WHERE id='.$_GET['id'];
        // 3 - Execution de la requête
        $req = mysqli_query($connexion, $sql) or die(mysqli_error($connexion));
        // 4 - traitement des donées
        $bloc = mysqli_fetch_array($req);
        ?>
        <form action="./core/update-formation-modal.php" method="POST">
